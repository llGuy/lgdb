...
void lgdb_single_source_step(struct lgdb_process_ctx *ctx) {
    ...
    if (current_line.SizeOfStruct && next_line.SizeOfStruct) {
        ...
        uint8_t current_line_first_instr;
        WIN32_CALL(ReadProcessMemory, ctx->proc_info.hProcess, (void *)current_line.Address, &current_line_first_instr, 1, &bytes_read);
        ...
        if (ctx->breakpoints.previous_line == current_line.LineNumber) {
            // __debugbreak();
        }

        uint32_t line_size = next_line.Address - ctx->thread_ctx.Rip;

        if (line_size == 1 && current_line_first_instr == 0xCC) {
