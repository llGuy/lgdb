...
struct TypeInfo 
{
	...
	{
		struct BaseTypeInfo         sBaseTypeInfo;     // If Tag == SymTagBaseType 
		struct TypedefInfo          sTypedefInfo;      // If Tag == SymTagTypedef 
		struct PointerTypeInfo      sPointerTypeInfo;  // If Tag == SymTagPointerType 
		struct UdtClassInfo         sUdtClassInfo;     // If Tag == SymTagUDT and UdtKind is "true" 
		struct UdtUnionInfo         sUdtUnionInfo;     // If Tag == SymTagUDT and UdtKind is "false" 
		struct BaseClassInfo        sBaseClassInfo;    // If Tag == SymTagBaseClass 
		struct EnumInfo             sEnumInfo;         // If Tag == SymTagEnum 
		struct ArrayTypeInfo        sArrayTypeInfo;    // If Tag == SymTagArrayType 
		struct FunctionTypeInfo     sFunctionTypeInfo; // If Tag == SymTagFunctionType 
		struct FunctionArgTypeInfo  sFunctionArgTypeInfo; // If Tag == SymTagFunctionArgType 
		struct DataInfo             sDataInfo;         // If Tag == SymTagData 
	} Info;
