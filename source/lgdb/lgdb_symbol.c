...
static uint32_t s_register_udt_type(lgdb_process_ctx_t *ctx, lgdb_symbol_type_t *type) {
    ...
    for (uint32_t i = 0; i < children_count; ++i) {
        ...
        if (tag == SymTagData) {
            ...
            base_classes[base_classes_count].size = length;
    ...
    type->uinfo.udt_type.base_classes_type_idx = children_storage + member_vars_size + methods_size;
...
static int s_print_data(lgdb_process_ctx_t *ctx, void *address, uint32_t size, lgdb_symbol_type_t *type) {
    ...
    case SymTagUDT: {
        ...
        for (int i = 0; i < type->uinfo.udt_type.base_classes_count; ++i) {
